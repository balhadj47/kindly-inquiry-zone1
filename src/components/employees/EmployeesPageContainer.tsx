...
const EmployeesPageContainer = () => {
  ...
  const handleRefresh = async () => {
    console.log('🔄 EmployeesPageContainer: Manual refresh triggered');
  ...
  const handleAddEmployee = () => {
    console.log('🆕 EmployeesPageContainer: Adding new employee');
    setSelectedEmployee(null);
    setIsModalOpen(true);
  };

  const handleEditEmployee = (employee: User) => {
    console.log('✏️ EmployeesPageContainer: Editing employee:', employee.id, employee.name);
    setSelectedEmployee(employee);
    setIsModalOpen(true);
  };

  const handleDeleteEmployee = (employee: User) => {
    console.log('🗑️ EmployeesPageContainer: Preparing to delete employee:', employee.id, employee.name);
    setSelectedEmployee(employee);
  ...
  const handleConfirmDelete = async () => {
    if (!selectedEmployee) {
      console.error('❌ EmployeesPageContainer: No employee selected for deletion');
      return;
    }
    
    try {
      console.log('🗑️ EmployeesPageContainer: Confirming deletion of employee:', selectedEmployee.id, selectedEmployee.name);
      await deleteUser.mutateAsync(selectedEmployee.id);
      
      console.log('✅ EmployeesPageContainer: Employee deleted successfully:', selectedEmployee.id);
      ...
      console.error('❌ EmployeesPageContainer: Error deleting employee:', error);
      toast({
        title: 'Erreur',
  ...
  const handleCancelDelete = () => {
    console.log('❌ EmployeesPageContainer: Canceling employee deletion');
    setIsDeleteDialogOpen(false);
    setSelectedEmployee(null);
  };
