...
export const createAddUserOperation = (setUsers: React.Dispatch<React.SetStateAction<User[]>>) => {
  const addUser = async (userData: UserOperationData) => {
    ...
    try {
      ...
      // Prepare data matching the actual database schema - add role field for type compatibility
      const roleMapping = {
        1: 'Administrator',
        2: 'Supervisor', 
        3: 'Employee'
      ...
      const insertData = {
        ...
        role: roleMapping[userData.role_id as keyof typeof roleMapping] || 'Employee',
        role_id: userData.role_id || 3,
        status: userData.status || 'Active',
