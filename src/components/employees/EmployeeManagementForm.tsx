...
const EmployeeManagementForm: React.FC<EmployeeManagementFormProps> = ({
  employee,
  onSubmit,
  isSubmitting,
  onCancel,
  ...
  useEffect(() => {
    ...
    if (currentEmployeeId !== formEmployeeId) {
      console.log('🔄 EmployeeManagementForm - Resetting form for new employee');
  ...
  const handleSubmit = async (data: FormData) => {
    try {
      console.log('🔍 EmployeeManagementForm - Raw form data:', data);
      
      // Clean and prepare the data for submission
      const submitData = prepareSubmitData(data);
      
      console.log('🚀 EmployeeManagementForm - Final submit data:', submitData);
      
      await onSubmit(submitData);
      console.log('✅ EmployeeManagementForm - Form submitted successfully');
      
      // Only reset form on successful submission
      form.reset();
      
    } catch (error) {
      console.error('❌ EmployeeManagementForm - Error submitting form:', error);
      // Don't reset form on error - let the error be displayed and form data preserved
    }
  };

  const handleImageChange = (url: string) => {
    console.log('🖼️ EmployeeManagementForm - Image changed to:', JSON.stringify(url));
