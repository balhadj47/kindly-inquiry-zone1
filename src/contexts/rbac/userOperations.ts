...
export const createUserOperations = (setUsers: React.Dispatch<React.SetStateAction<User[]>>) => {
  ...
  const changeUserPassword = async (userEmail: string, newPassword: string) => {
    ...
    try {
      ...
      // Always try to send a password reset email first as it's more reliable
      ...
      if (resetError) {
        ...
        // If reset email fails, try admin API as fallback
        if (userData?.auth_user_id) {
          ...
          // User has no auth account, create one
          ...
          console.log('New auth account created successfully');
          return;
        }
      }

      // If reset email was sent successfully
